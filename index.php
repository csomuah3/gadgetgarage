...
<head>
	...
	.hero-slide {
		...
		@keyframes kenBurnsZoomPan {
			0% {
				transform: translateY(-120px) scale(1.3) translateX(-3%);
				opacity: 0;
			}
...
<body>
	...
	<script>
		...
		async function loadAIRecommendations() {
			...
			try {
				const response = await fetch('../actions/get_ai_recommendations.php');
				const data = await response.json();
		...
		function createProductCard(product) {
			...
			card.onclick = function() {
				window.location.href = `views/single_product.php?pid=${product.product_id}`;
			};
			...
			card.innerHTML = `
				<div style="width: 100%; height: 250px; background: #f8fafc; display: flex; align-items: center; justify-content: center; overflow: hidden;">
					<img src="${imageUrl}" alt="${product.product_title || 'Product'}" 
				...
				<div style="padding: 20px;">
					...
					<button onclick="event.stopPropagation(); window.location.href='views/single_product.php?pid=${product.product_id}'" 
